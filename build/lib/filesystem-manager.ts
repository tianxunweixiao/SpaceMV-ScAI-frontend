...
export class FileSystemManager {
  ...
  /**
   * Updates login.html with the tianxun server URL
   * @param sourcePath The source login.html file path
   * @param destPath The destination login.html file path
   * @param tianxunServerUrl The tianxun server URL to inject
   */
  updateLoginHtml(sourcePath: string, destPath: string, tianxunServerUrl: string): void {
    const fullSourcePath = this.resolvePath(sourcePath);
    const fullDestPath = this.resolvePath(destPath);

    tryCatchWithBuildError(
      () => {
        let content = readFileSync(fullSourcePath, 'utf-8');
        content = content.replace(/\$\{TIANXUN_SERVER_LOGIN\}/gu, tianxunServerUrl);
        writeFileSync(fullDestPath, content, 'utf-8');
        logWithStyle(`Updated login.html with tianxun server: ${tianxunServerUrl}`, ConsoleStyles.DEBUG);
      },
      `Failed to update login.html: ${sourcePath}`,
      ErrorCodes.FILE_OPERATION,
    );
  }
}
