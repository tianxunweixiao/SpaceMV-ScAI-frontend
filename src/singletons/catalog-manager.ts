...
export class CatalogManager {
  ...
  updateCruncherBuffers = (mData: SatCruncherMessageData): void => {
    ...
    // 获取dots-manager中的星星索引范围
    const dotsManager = keepTrackApi.getDotsManager();
    const starIndex2 = dotsManager.starIndex2 || 0; // 星星的最大ID

    // 修复：确保dotsOnScreen至少包含所有星星的索引范围，并且减去maxFieldOfViewMarkers后不会得到负值
    settingsManager.dotsOnScreen = Math.max(Math.max(this.numObjects - settingsManager.maxFieldOfViewMarkers, 0), highestMarkerNumber, starIndex2);
