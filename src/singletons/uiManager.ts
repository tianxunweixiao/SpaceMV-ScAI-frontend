...
export class UiManager {
  ...
  initMenuController() {
    ...
    getEl('legend-menu')?.addEventListener('click', () => { // TODO: Remove this
      this.initAllLayersToOff();
  ...
  initAllLayersToOff() {
    setTimeout(() => {
      ...
      if (colorSchemeManagerInstance && colorSchemeManagerInstance.objectTypeFlags) {
        const layerTypes = Object.keys(colorSchemeManagerInstance.objectTypeFlags);

        layerTypes.forEach((slug) => {
          // 设置所有图层为关闭状态
          ...
          // 同步更新颜色方案实例
          if (colorSchemeInstance) {
            colorSchemeInstance.objectTypeFlags[slug] = false;
          }
        });
    ...
    setTimeout(() => {
      ...
      if (colorSchemeManagerInstance && colorSchemeManagerInstance.objectTypeFlags) {
        ...
        layerTypes.forEach((slug) => {
          ...
          // 更新UI显示
          getClass(`legend-${slug}-box`).forEach((el) => {
            if (el && colorSchemeInstance?.colorTheme[slug]) {
              el.style.background = rgbCss(colorSchemeInstance.colorTheme[slug]);
            }
          });

          // 同步更新颜色方案实例
          if (colorSchemeInstance) {
            colorSchemeInstance.objectTypeFlags[slug] = true;
          }
        });
