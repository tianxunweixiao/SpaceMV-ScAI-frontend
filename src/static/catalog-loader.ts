...
export interface Constellation {
  ...
  constellation_type?: string; // 1: 通信星座, 2: 导航星座, 3: 对地观测星座, 4: 自定义星座
}

// 全局缓存变量
export let cachedConstellationsData: Constellation[] = [];
let constellationsLoading = false;
...
export class CatalogLoader {
  static filterTLEDatabase(resp: KeepTrackTLEFile[], extraSats: ExtraSat[] | null, asciiCatalog: AsciiTleSat[] | null, jsCatalog: JsSat[] | null): void {
    ...
    //note by Pengy 定期刷新时，应该修改成增量更新机制
  ...
  static async load(): Promise<void> {
    ...
    try {
      ...
      //     await fetch(settingsManager.dataSources.tianxunServer + '/satellites')
      //       .then((response) => {
        ...
        await fetch(settingsManager.dataSources.tianxunServer + '/satellites')
          .then((response) => {
            if (response.status === 500) {
              throw new Error('ServerError500');
            }
            if (!response.ok) {
          ...
          .catch(async (error) => {
            if (error.message === 'ServerError500') {
              errorManagerInstance.error(error, '服务器返回500错误，请稍后再试');
              ...
              errorManagerInstance.error(error, '获取行星数据失败');
            }
          });
      // }
    } catch (e) {
  ...
  private static addNonSatelliteObjects_(catalogManagerInstance: CatalogManager, tempObjData: BaseObject[]) {
    ...
    catalogManagerInstance.orbitalSats = tempObjData.length;
  ...
  private static processAsciiCatalog_(asciiCatalog: AsciiTleSat[], catalogManagerInstance: CatalogManager, tempSatData: DetailedSatellite[]) {
    if (settingsManager.dataSources.externalTLEs) {
      if (settingsManager.dataSources.externalTLEs !== 'https://storage.keeptrack.space/data/celestrak.txt' && !settingsManager.dataSources.externalTLEs.startsWith('blob:')) {
    ...
    if (settingsManager.dataSources.externalTLEs) {
      if (settingsManager.dataSources.externalTLEsOnly && !settingsManager.dataSources.isSupplementExternal) {
   ...
   * @returns 星座数据数组
   */
  static async fetchAndCacheConstellations(): Promise<Constellation[]> {
    // 检查是否已经在加载中
    if (constellationsLoading) {
      // 等待加载完成
      while (constellationsLoading) {
        await new Promise(resolve => setTimeout(resolve, 100));
      }
      return cachedConstellationsData;
    }

    constellationsLoading = true;
    try {
      // 调用星座数据API
      const response = await fetch( settingsManager.dataSources.tianxunServer + '/constellations');
      if (!response.ok) {
        throw new Error(`Failed to fetch constellations: ${response.status}`);
      }
      ...
      // 缓存数据并更新加载时间
      cachedConstellationsData = data;
      
      return cachedConstellationsData;
    } catch (error) {
      errorManagerInstance.error(
        error instanceof Error ? error : new Error('Failed to fetch constellations'), 
        'CatalogLoader.fetchAndCacheConstellations'
      );
      
      // 如果发生错误但已有缓存数据，返回缓存数据
      if (cachedConstellationsData.length > 0) {
        return cachedConstellationsData;
      }
   ...
   * @returns 缓存的星座数据数组
   */
  static getCachedConstellations(): Constellation[] {
    return cachedConstellationsData;
  }
   ...
   * 清除星座数据缓存
   */
  static clearConstellationsCache(): void {
    cachedConstellationsData = [];
  }
}
